<?php
// +----------------------------------------------------------------------
// | JoelCMS [ For Memory For Joel. ]
// +----------------------------------------------------------------------
// | Copyright (c) 2013 http://www.JoelCMS.com All rights reserved.
// +----------------------------------------------------------------------
// | Author: Joel <2094157689@qq.com> <http://www.JoelCMS.com>
// +----------------------------------------------------------------------

// JOELCMS常量定义
const JOELCMS_VERSION    = '1.0.1';
const JOELCMS_ADDON_PATH = './Addons/';
...
function wxpay($openId,$goods,$order_sn,$total_fee,$attach){  
    ...
    require_once APP_ROOT."/Api/wxpay/example/WxPay.JsApiPay.php";  
    require_once APP_ROOT.'/Api/wxpay/example/payment/log.php';  
 ...
 * @author Joel <2094157689@qq.com>
 */
function joelTree($m,$pid,$field,$map,$order='sorts desc',$keyid='id',$keypid='pid',$keychild='_child'){
 ...
 */
  ...
  * 获取系统参数
  */
 function sysconfig($k){
 	$obj=M('sys_config')->where("`key`='".$k."'")->find();
	if($obj['status']==1){
		return $obj['value'];
	}else{
		return '此参数已停用';
...
function makecode($qrcode_path,$content,$matrixPointSize,$matrixMarginSize,$errorCorrectionLevel,$url){
    ...
    $object = new \QRcode();
    $qrcode_path_new = './Public/S/images/code'.'_'.date("Ymdhis").'.png';//定义生成二维码的路径及名称
