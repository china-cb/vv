...
class UgoodsController extends BaseController {
	...
	public function ads(){
		...
		//$map['a.uid']=array("eq",intval(self::$S['uid']));
		$map['b.status']=array("eq",1);

		$psize=self::$S['set']['pagesize']?self::$S['set']['pagesize']:20;
		$cache=$m->where($map)->alias("a")->field("a.h5_url,h5_qrimg,b.*")->join("left join joel_shop_goods b on a.goodsid=b.id and a.uid=".self::$S['uid'])->where($map)->page($p,$psize)->select();
		//echo $r = M()->getLastSql();
		//var_dump($cache);
		...
		$count=$m->where($map)->alias("a")->field("a.h5_url,h5_qrimg,b.*")->join("left join joel_shop_goods b on a.goodsid=b.id and a.uid=".self::$S['uid'])->where($map)->count();
...
//我的资产
	public function zc(){
		...
		$cache[money]=$fxs['money'];
//获取提现金额		
		$st = 1;
		$txs=$tx->where("sid='$uid' AND status='$st'")->select();
		$cache[txprice]=$txs[0]['txprice'];			
//获取全部收益
		$this->assign('cache',$cache);			
  		$this->display();
	}
