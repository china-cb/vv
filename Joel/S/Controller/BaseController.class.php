...
class BaseController extends Controller {
    ...
    protected function _initialize(){
    	...
    	//刷新全局配置
    	self::$S['set']=$_SESSION['S']['set']=$this->checkSet();
		//检测登陆状态
	...
	//全局JoelInit方法
	public function JoelInit(){
		self::$S['uid']=$_SESSION['S']['uid'];
		self::$S['user']=$_SESSION['S']['user'];
	...
	public function checkLogin(){
		...
		if(!$check){
			if(!isset($_SESSION['S']['uid'])){
				$this->error('登陆已超时,请重新登陆',U('S/Public/login'));
			}
		}else{
			return TRUE;
		}		
