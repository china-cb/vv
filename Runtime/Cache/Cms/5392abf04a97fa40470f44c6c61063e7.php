		...
		<div class="widget">
			...
			<div class="widget-body">
				<div class="table-toolbar">
					<a href="<?php echo U('Cms/Fxs/userSet/');?>" class="btn btn-primary" data-loader="Joel-loader" data-loadername="设置分组">
						<i class="fa fa-plus"></i>新增分销商
					</a>
					<div class="pull-right">
						<form id="Joel-search">
							<label>
								<select name="stype" class="form-control input-sm">
									<option value='1' <?php if(($stype) == "1"): ?>selected<?php endif; ?>>按分销商姓名【模糊】</option>
									<option value='2' <?php if(($stype) == "2"): ?>selected<?php endif; ?>>按分销商ID</option>
									<option value='3' <?php if(($stype) == "3"): ?>selected<?php endif; ?>>按分销商ID搜索下级</option>
								</select>
							</label>
							<label><input name="search" type="search" class="form-control input-sm" placeholder="请输入搜索条件"></label>
							<a href="<?php echo U('Cms/Fxs/user/');?>" class="btn btn-success" data-loader="Joel-loader" data-loadername="分销商管理" data-search="Joel-search">
				...
				<table id="Joel-table" class="table table-bordered table-hover">
					...
					<tbody>
						<?php if(is_array($cache)): $i = 0; $__LIST__ = $cache;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr id="item<?php echo ($vo["id"]); ?>">
								...
								<td class=" "><a href="<?php echo U('Cms/Shop/order',array('id'=>$vo['id']));?>" class="btn btn-success btn-xs" data-loader="Joel-loader" data-loadername="分销课程管理-全部课程">查看课程信息</a></td>
								<td class="center "><a href="<?php echo U('Cms/Fxs/userSet/',array('id'=>$vo['id']));?>" class="btn btn-success btn-xs" data-loader="Joel-loader" data-loadername="设置分销商"><i class="fa fa-edit"></i> 编辑</a></td>
							</tr>
							<?php if(is_array($vo['_child'])): $i = 0; $__LIST__ = $vo['_child'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo2): $mod = ($i % 2 );++$i;?><tr id="item<?php echo ($vo2["id"]); ?>">
									...
									<td class="center "><a href="<?php echo U('Cms/Fxs/userSet/',array('id'=>$vo2['id']));?>" class="btn btn-success btn-xs" data-loader="Joel-loader" data-loadername="设置分销商"><i class="fa fa-edit"></i> 编辑</a></td>
								</tr>
								<?php if(is_array($vo2['_child'])): foreach($vo2['_child'] as $key=>$vo3): ?><tr id="item<?php echo ($vo3["id"]); ?>">
									...
									<td class="center "><a href="<?php echo U('Cms/Fxs/userSet/',array('id'=>$vo3['id']));?>" class="btn btn-success btn-xs" data-loader="Joel-loader" data-loadername="设置分销商"><i class="fa fa-edit"></i> 编辑</a></td>
									</tr>	
									<?php if(is_array($vo3['_child'])): foreach($vo3['_child'] as $key=>$vo4): ?><tr id="item<?php echo ($vo4["id"]); ?>">
										...
										<td class=" "><?php echo ($vo4["no"]); ?></td>
										<td class=" ">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;└<?php echo ($vo4["nickname"]); ?></td>
										<td class=" "><?php echo ($vo4["mobile"]); ?></td>
										<td class=" "><?php echo ($vo4["username"]); ?></td>		
										<td class=" ">0</td>	
										<td class=" "><?php echo ($vo4["total_xxyj"]); ?>元</td>
										<td class=" "><?php echo ($vo4["total_xxlink"]); ?>人</td>
										<td class=" "><?php echo ($vo4["total_xxsub"]); ?>人</td>
										<td class=" "><?php echo ($vo4["total_xxbuy"]); ?>次</td>
										<td class=" "><?php echo (date('Y-m-d',$vo4["ctime"])); ?></td>
										<td class=" "><button class="btn btn-xs btn-blue showqr" data-title="<?php echo ($vo4["nickname"]); ?>" data-qr="<?php echo ($vo4["qrticket"]); ?>">点击查看</button></td>
										<td class=" "><?php if(($vo4["status"]) == "1"): ?><button class="btn btn-xs btn-success">正常</button><?php else: ?><button class="btn btn-xs btn-danger">停用</button><?php endif; ?></td>
										<td class="center "><a href="<?php echo U('Cms/Fxs/userSet/',array('id'=>$vo4['id']));?>" class="btn btn-success btn-xs" data-loader="Joel-loader" data-loadername="设置分销商"><i class="fa fa-edit"></i> 编辑</a></td>
										</tr><?php endforeach; endif; endforeach; endif; endforeach; endif; else: echo "" ;endif; endforeach; endif; else: echo "" ;endif; ?>
